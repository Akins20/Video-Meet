...
export interface RecordingStatus {
    id: string
    meetingId: string
    status: 'starting' | 'recording' | 'paused' | 'stopping' | 'processing' | 'completed' | 'failed'
...
export const isParticipantModerator = (participant: MeetingParticipant): boolean => {
...
export const getMeetingDuration = (meeting: Meeting): number => {
    ...
    return Math.floor((endTime.getTime() - new Date(meeting.startedAt).getTime()) / 1000)
}

export const getParticipantSessionDuration = (participant: MeetingParticipant): number => {
    if (!participant.joinedAt) return 0
    const endTime = participant.leftAt ? new Date(participant.leftAt) : new Date()
    return Math.floor((endTime.getTime() - new Date(participant.joinedAt).getTime()) / 1000)
}