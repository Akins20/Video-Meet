...
const MeetingList: FC = () => {
    ...
    // Deduplicate meetings and create stable keys - FIX FOR DUPLICATE KEY ERROR
    const uniqueMeetings = useMemo(() => {
        const meetings = data?.data?.meetings || [];
        const seen = new Set();
        return meetings.filter((meeting: any) => {
            if (seen.has(meeting.id)) {
                console.warn(`Duplicate meeting ID found: ${meeting.id}`);
                return false;
            }
            seen.add(meeting.id);
            return true;
    ...
    const isCurrentUserHost = (meeting: any) => {
        // Handle both object and string hostId formats
    ...
    return (
        <>
            ...
            >
                <AnimatePresence mode="popLayout">
                    {uniqueMeetings.map((meeting, index) => {
                        ...
                        const uniqueKey = `${meeting.id}-${index}`;
            ...
            <AnimatePresence>
                {meetingToEnd && (
                    <EndMeetingDialog
                        meetingTitle={uniqueMeetings.find(m => m.id === meetingToEnd)?.title || "Unknown Meeting"}
