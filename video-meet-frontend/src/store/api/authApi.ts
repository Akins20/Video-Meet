...
export const authApiUtils = {
  // Prefetch user data
  prefetchUserData: () =>
    authApi.util.prefetch("getCurrentUser", undefined, { force: false }),
    ...
    return (username: string, callback: (available: boolean) => void) => {
      ...
      timeoutId = setTimeout(async () => {
        try {
          ...
          // @ts-expect-error
          const result = await (await import("../store")).default
            .dispatch(
              authApi.endpoints.checkUsernameAvailability.initiate(username)
            )
            .unwrap();
    ...
    return (email: string, callback: (available: boolean) => void) => {
      ...
      timeoutId = setTimeout(async () => {
        try {
          ...
          // @ts-expect-error
          const result = await (await import("../store")).default
            .dispatch(authApi.endpoints.checkEmailAvailability.initiate(email))
            .unwrap();
          callback(result.data?.available || false);
        } catch {
          callback(false);
