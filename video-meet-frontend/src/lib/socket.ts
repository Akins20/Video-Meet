...
class SocketClient {
     ...
     * Update media state
     */
    public updateMediaState(roomId: string, mediaState: {
    ...
    private handleReconnectError(error: Error): void {
        ...
        if (this.connectionState.reconnectAttempts >= this.config.options.maxReconnectionAttempts) {
    ...
    private scheduleReconnect(): void {
        ...
        const delay = Math.min(
            this.config.options.reconnectionDelay * Math.pow(2, this.connectionState.reconnectAttempts),
    ...
    private emitConnectionStateChange(): void {
        ...
        if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('socket:connection-state-change', {
...
export const socket = {
    ...
    updateConnectionQuality: socketClient.updateConnectionQuality.bind(socketClient),
...
export const socketUtils = {
    ...
    getConnectionStats: () => {
        ...
        return {
            ...
            downtime: state.lastDisconnected ? Date.now() - state.lastDisconnected.getTime() : 0,
        }
    },
